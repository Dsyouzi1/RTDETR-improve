...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz=640, p=1.0, n=4, label_mask=0):
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if self.label_mask:
                mask_label = np.full((s * 2, s * 2, 4), 114, dtype=float)
                for j in range(4):
                    mask_label[y1a:y2a, x1a:x2a, j] = labels_patch['imParams'][j]
            ...
            mosaic_labels.append(labels_patch)
        if self.label_mask:
            final_labels['label_mask'] = mask_label
            final_labels['center_point'] = (yc, xc)
        final_labels = self._cat_labels(mosaic_labels)
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        for labels in mosaic_labels:
            ...
            imparams.append(labels['imParams'])
        ...
        final_labels = {
            ...
            "imParams": np.concatenate(imparams, 0)
        }
...
class RandomPerspective:
    ...
    def __init__(
        self, degrees=0.0, translate=0.1, scale=0.5, shear=0.0, perspective=0.0, border=(0, 0), pre_transform=None, label_mask=0
    ):
    ...
    def __call__(self, labels):
        ...
        if self.label_mask:
            masked_label = labels['label_mask']
            #对labelmask做变换 同时对中心点也做变换 与图像映射
            labelmask = cv2.warpAffine(masked_label, M[:2], dsize=self.size, borderValue=(114, 114, 114))
            (yc, xc) = labels['center_point']
            center_point = np.array([xc, yc, 1])  # 将中心点转换为齐次坐标
            new_center_point = M @ center_point  # 通过仿射变换矩阵变换中心点
            new_xc, new_yc = np.round(new_center_point[:2]).astype(int)  # 得到变换后的整数中心点
            labels['center_point'] = (new_yc,new_xc)
            labels['label_mask'] = labelmask
...
class RandomFlip:
    ...
    def __init__(self, p=0.5, direction="horizontal", flip_idx=None, label_mask=0) -> None:
        """
    ...
    def __call__(self, labels):
        ...
        img = labels["img"]
        if self.label_mask:
            label_mask = labels["label_mask"]  # 飞行参数标签
        instances = labels.pop("instances")
        ...
        if self.direction == "vertical" and random.random() < self.p:
            ...
            instances.flipud(h)
            if self.label_mask:
                label_mask = np.flipud(label_mask)  # 对飞行参数标签进行上下翻转

        if self.direction == "horizontal" and random.random() < self.p:
            img = np.fliplr(img)
            instances.fliplr(w)
            if self.label_mask:
                label_mask = np.fliplr(label_mask)  # 对飞行参数标签进行上下翻转
...
class Format:
    ...
    def __call__(self, labels):
        ...
        if self.label_mask:
            label_mask = labels.pop('label_mask')
            labels['label_mask'] = self._format_label_mask(label_mask)
        ...
        labels['imParams'] = torch.from_numpy(np.array(imParams))
    ...
    def _format_label_mask(self, label_mask):
        """Format the image for YOLO from Numpy array to PyTorch tensor."""
        
        label_mask = label_mask.transpose(2, 0, 1)
        label_mask = np.ascontiguousarray(label_mask)
