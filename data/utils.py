...
def read_exif_user_comment(image_path):
    ...
    numbers = re.findall(r"[-+]?\d+\.\d+|[-+]?\d+", decoded_str)
    float_numbers = []
    # 将提取的数字转换为浮点数
    float_numbers = []
    for num in numbers:
        try:
            ...
            pass
    # 焦距 飞行高度 激光测距 俯仰角
    # return [min(max((float_numbers[0] // 12) - 1, 0), 2) ,(float_numbers[1]-31.768)/(121.397-31.768), (float_numbers[3]+90.1)/(-0.1+90.1)]
    return [min(max((float_numbers[0] // 12) - 1, 0), 2) ,float_numbers[1], float_numbers[3]]


def img2label_paths(img_paths):
    """Define label paths as a function of image paths."""
    sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}HBB{os.sep}"  # /images/, /labels/ substrings
    return [sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt" for x in img_paths]

def imgr2label_paths(img_paths):
    """Define label paths as a function of image paths."""
    sa, sb = f"{os.sep}imagesr{os.sep}", f"{os.sep}HBBR{os.sep}"  # /images/, /labels/ substrings
    return [sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt" for x in img_paths]
