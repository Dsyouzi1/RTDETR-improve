...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        targets = {
            ...
            'gt_groups': gt_groups,
            "params": batch['params'].to(img.device)
            }
    ...
    def predict(self, x, profile=False, visualize=False, batch=None, augment=False):
        ...
        for m in self.model[:-1]:  # except the head part
            ...
            if hasattr(m, 'backbone'):
                ...
                if isinstance(m, (AIFI, Blocks_addParams)):
                    x = m([x, params])
                else:
                    x = m(x)  # run
                y.append(x if m.i in self.save else None)  # save output
        ...
        x = head([[y[j] for j in head.f], params], batch)  # head inference
        return x
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 C2f_EfficientVIM, C2f_EfficientVIM_CGLU, CSP_MSCB_SC, C2f_MambaOut_UniRepLK, C2f_IEL, IELC3, C2f_RCB):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, DWRC3, C3_DWR, C2f_DWR, C3_DCNv2_Dynamic, C2f_DCNv2_Dynamic,
                ...
                n = 1
                   ...
                   TransformerEncoderLayer_Pola, TransformerEncoderLayer_TSSA, TransformerEncoderLayer_ASSA):
            c2 = ch[f]
            args = [ch[f], *args]
        ...
        # --------------ASF--------------
            ...
            c2 = c1[1]
            args = [c1, *args[1:]]
        elif m in [Blocks, Blocks_addParams]:
